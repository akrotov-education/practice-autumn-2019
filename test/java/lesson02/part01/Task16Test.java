...
public class Task16Test {
    ...
    public void FirstSymbol() {
        ...
        Assert.assertTrue("Текст должен начинаться с \"日\"", m.find());
    }
    ...
    public void LastSymbol() {
        ...
        Matcher m = p.matcher(s);
        Assert.assertTrue("Текст должен заканчиваться на \"語\"", m.find());
    }

    @Test
    public void RightOutputString() {
        Task16.main(null);
        SystemOutGatewayUtil.setOriginalOut();
        ByteArrayOutputStream outputArr = SystemOutGatewayUtil.getOutputArray();
        String s = outputArr.toString();
        Assert.assertTrue("Выведенный текст должен соответствовать заданию", s.split("\r\n")[0].equals("日本語"));
    }
}