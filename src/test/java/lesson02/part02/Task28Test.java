...
public class Task28Test {
    ...
    public void task28isResultCorrect() throws Exception {
        ...
        Assert.assertEquals("Программа должна выводить количество положительных чисел в исходном наборе.","3",result);
        stream.reset();
    }
    ...
    public void task28ifZeroIsntCounted() throws Exception {
        ...
        ByteArrayOutputStream stream = SystemOutGatewayUtil.getOutputArray();
        String result = stream.toString();
        Assert.assertEquals("0 не должен идти в счётчик положительных чисел!","0",result);
        stream.reset();
    }
}