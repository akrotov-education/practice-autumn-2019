...
public class Task02Test {
    ...
    public void task02InputStringsToList() {
        boolean inp = false;
        for (String s : collect) {
            if (s.contains("int i = 0; i < list.length-2; i++") || s.contains("list[i] = reader.readLine()"))
                inp = true;
        }
    ...
    public void task02CheckOutput() {
        ...
        for (String s : collect) {
            if (s.contains("System.out.println(list[list.length - 1-i])")) {
                checkout = true;
            } else if (s.contains("int i = 0; i < list.length; i++")) {
                checkout = true;
            }
        }
