...
public class Task10Test {
    ...
    private static String fileName = "./src/main/java/lesson01/part1/Task10.java";
    ...
    public void task10isOutputPositiveInteger() {
        ...
        int value = 0;
        try {
            value = Integer.parseInt(result.substring(0, result.length()-2));
        } catch (Exception e) {
    ...
    public void task10isOutputCorrect() {
        ...
        ByteArrayOutputStream stream = SystemOutGatewayUtil.getOutputArray();
        String result = stream.toString();
        Assert.assertEquals("Должно получиться 1800 секунд.", "1800\r\n", result);
        stream.reset();
    }
}